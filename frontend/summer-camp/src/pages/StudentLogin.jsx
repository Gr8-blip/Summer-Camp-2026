...
export default function StudentLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Student login has no JWT in your current backend — store basic info locally
      localStorage.setItem("student_info", JSON.stringify(data));
      navigate("/dashboard");
    } catch (err) {
      setError(err.data?.error || err.message || "Hmm, that code didn't work!");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page-shell login-page">
      ...
      <div className="login-container fade-up">
        <div className="login-card">
          {/* Loud tab switcher so parents/students never go hunting for the other link */}
